...
square length center rotation = Polygon $ map ((center &+) . (length/2 *&) . rotate2 rotation) unitSquare
...
rotate3d (a,b) = fmap $ rotate3 (pi/2*b) (Vec3 0 1 0) . rotate3 (pi/2*a) (Vec3 1 0 0)

-- |A cube of length, position and rotation represented by a list of Vec3 polygons
cube :: Float -> Vec3 -> Rot3 -> [Polygon Vec3]
cube length center (Rot3 axis rotation) = fmap (fmap $ rotate3 rotation axis) sides
                                          where sides = rotators <*> [side] :: [Polygon Vec3]
                                                rotators = ([rotate3d] <*> rotations) :: [Polygon Vec3 -> Polygon Vec3]
                                                rotations = [(0,0),(-1,0),(1,0),(2,0),(0,-1),(0,1)] :: [(Float,Float)]
                                                -- Creates a side which is then replicated and rotated around
                                                side = embed (length/2) $ square length (Vec2 0 0) 0 :: Polygon Vec3
